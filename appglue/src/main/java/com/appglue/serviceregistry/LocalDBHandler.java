...
import static com.appglue.Constants.DELIMITER;
...
import static com.appglue.Constants.PARAMETER_ID;
import static com.appglue.Constants.PARAM_REQUIREDNESS;
import static com.appglue.Constants.PARAM_TYPE;
import static com.appglue.Constants.PARENT_SERVICE;
import static com.appglue.Constants.POSITION;
import static com.appglue.Constants.POSS_SYSTEM;
import static com.appglue.Constants.POSS_USER;
...
import static com.appglue.Constants.USE_DEFAULT;
...
public class LocalDBHandler extends SQLiteOpenHelper
{
	...
	private void create(SQLiteDatabase db)
	{
		...
		db.execSQL(String.format("DROP TABLE IF EXISTS %s", TBL_ATOMIC));
		db.execSQL(AppGlueLibrary.createTableString(TBL_ATOMIC, COLS_ATOMIC));
		
		db.execSQL(String.format("DROP TABLE IF EXISTS %s", TBL_APP));
		db.execSQL(AppGlueLibrary.createTableString(TBL_APP, COLS_APP));
		
		db.execSQL(String.format("DROP TABLE IF EXISTS %s", TBL_COMPOSITE_HAS_ATOMIC));
		db.execSQL(AppGlueLibrary.createTableString(TBL_COMPOSITE_HAS_ATOMIC, COLS_COMPOSITE_HAS_ATOMIC));
	...
	public long addAtomic(ServiceDescription sd)
	{
		...
		try
		{
			retval = db.insertOrThrow(TBL_ATOMIC, null, values);
	...
	public ArrayList<ServiceDescription> getComponentsForApp(String packageName)
	{
		...
		String query = String.format("SELECT * FROM %s WHERE %s = \"%s\"", TBL_ATOMIC, PACKAGENAME, packageName);
	...
	{		
		...
		int retval = db.update(TBL_ATOMIC, args, strFilter, null);
	...
	public ArrayList<ServiceDescription> getComponents(ProcessType type)
	{
		ArrayList<ServiceDescription> serviceList = new ArrayList<ServiceDescription>();
		SQLiteDatabase db = this.getReadableDatabase();
		
		Cursor c = db.query(TBL_ATOMIC, null,
				PROCESS_TYPE + " = ?", new String[] { "" + ProcessType.TRIGGER.index },
				null, null, null, null);
		...
		if(c == null)
		{
			Log.e(TAG, String.format("Component cursor null: %s", "getAtomics(Process)"));
			return serviceList;
		}
		
		if(c.getCount() == 0)
		{
			Log.e(TAG, String.format("Component cursor empty: %s", "getAtomics(Process)"));
			return serviceList;
		}
	...
	/**
	 * Gets all of the atomic services we know about with the given ServiceType 
	 * 
	...
	public ArrayList<ServiceDescription> getAtomics(ServiceType type, boolean includeConverters)
	{
		...
		if(type == ServiceType.LOCAL)
			typeString = String.format(" WHERE %s.%s = %s", TBL_ATOMIC, SERVICE_TYPE, ServiceType.LOCAL.index);
		else if(type == ServiceType.IN_APP)
			typeString = String.format(" WHERE %s.%s = %s", TBL_ATOMIC, SERVICE_TYPE, ServiceType.IN_APP.index);
		else if(type == ServiceType.DEVICE)
			typeString = String.format(" WHERE %s.%s = %s OR %s.%s = %s", TBL_ATOMIC, SERVICE_TYPE, ServiceType.LOCAL.index, TBL_ATOMIC, SERVICE_TYPE, ServiceType.IN_APP.index);
		
		String query = String.format("SELECT * FROM %s %s", TBL_ATOMIC, typeString);
		...
		if(c == null)
		{
			Log.e(TAG, String.format("Atomic cursor null: %s", "getAtomics(Service)"));
			return serviceList;
		}
		
		if(c.getCount() == 0)
		{
			Log.e(TAG, String.format("Atomic cursor empty: %s", "getAtomics(Service)"));
			return serviceList;
		}
		...
		do
		{
			...
			ArrayList<ServiceIO> inputs = getServiceIOs(sd, true);
			if(inputs == null) inputs = new ArrayList<ServiceIO>();
			sd.setInputs(inputs);
			
			ArrayList<ServiceIO> outputs = getServiceIOs(sd, false);
			if(outputs == null) outputs = new ArrayList<ServiceIO>();
			sd.setOutputs(outputs);
...
//			if(!sd.isConverter() || includeConverters)
				serviceList.add(sd);
		}
		while(c.moveToNext());
	...
	private ArrayList<ServiceIO> getServiceIOs(ServiceDescription sd, boolean input)
	{
		...
		Cursor c = db.query(TBL_SERVICEIO, null,
	 			PARENT_SERVICE + " = ? AND " + I_OR_O + " = ?", new String[] { sd.getClassName(),  textInput},
				null, null, null, null);
	...
	public int deleteRemoteAtomics()
	{
		...
		return db.delete(TBL_ATOMIC, "? = ?", new String[] { SERVICE_TYPE, "" + ServiceType.REMOTE.index });
	}
	...
	public int deleteDeviceAtomics()
	{
		...
		return db.delete(TBL_ATOMIC, SERVICE_TYPE + " = ?", new String[] { "" + ServiceType.LOCAL.index });
	}
	...
	public long updateComposite(CompositeService cs) 
	{
		...
		else
		{
			...
			int num = db.delete(TBL_COMPOSITE_HAS_ATOMIC, COMPOSITE_ID + " = ?", new String[]{ "" + cs.getId() });
			for(int i = 0; i < components.size(); i++)
			{
			...
			num = db.delete(TBL_COMPOSITE_IOCONNECTION,  COMPOSITE_ID + " = ?", new String[]{ "" + cs.getId() });
	...
	private boolean addIOConnections(long compositeId, ArrayList<ServiceDescription> services)
	{
		...
		for(int i = 0; i < services.size(); i++)
		{
			...
			{
				ServiceIO input = currentInputs.get(j);
				ServiceIO output = input.getConnection();
				
				if(output == null) // Currenty nothing is mandatory...
					continue;
...
//	public boolean updateCompositeDetails(long id, String name, String description)
//	{
//		SQLiteDatabase db = this.getReadableDatabase();
//		
...
//		int numRows = db.update(TBL_COMPOSITE, args, strFilter, null);
//		
//		if(numRows == 0)
//			return false;
//		else
...
//	}
	...
	public ArrayList<CompositeService> getComposites()
	{
		...
		Cursor c = db.query(TBL_COMPOSITE, new String[] { ID },	null, null, null, null, null);
	...
	public void setupComponents(CompositeService cs)
	{
		...
		Cursor c = db.query(TBL_COMPOSITE_HAS_ATOMIC, null,
	...
	public void setupConnections(CompositeService cs)
	{
		...
		do
		{
			...
			int outputId = c.getInt(c.getColumnIndex(OUTPUT_IO_ID));
			...
			int inputId = c.getInt(c.getColumnIndex(INPUT_IO_ID));
	...
	public boolean deleteComposite(CompositeService cs)
	{
		...
		SQLiteDatabase db = this.getWritableDatabase();
		int status = db.delete(TBL_COMPOSITE, ID + "=?", new String[] { "" + id });
		status = db.delete(TBL_COMPOSITE_HAS_ATOMIC, ID +"=?", new String[]{ "" + id });
		status = db.delete(TBL_COMPOSITE_IOCONNECTION, ID + "=?", new String[] { "" + id });
		status = db.delete(TBL_FILTER, ID + "=?", new String[] { "" + id });
		
		// Not sure what to do with status
	...
	public long addCompositeHasAtomic(long compositeId, String className, int position)
	{
		...
		return db.insert(TBL_COMPOSITE_HAS_ATOMIC, null, values);
	}
	...
	{		
		...
		// Delete all of the filters for that partcular composite 
...
//			try 
//			{
//				Class<?> clazz = Class.forName(io.getType().getClass().getCanonicalName());
//				CompositionDatatype cd = (CompositionDatatype) clazz.newInstance();
//				io.setManualValue(cd.reloadValue(manualValue));
//			}
...
//			}
	...
	public ArrayList<CompositeService> atomicAtPosition(String className, int position) 
	{
		...
		String sql = String.format("SELECT DISTINCT %s FROM %s WHERE %s = '%s' AND %s = %s", COMPOSITE_ID, TBL_COMPOSITE_HAS_ATOMIC, CLASSNAME, className, POSITION, position);
	...
	{
String query = String.format("SELECT * FROM %s WHERE %s = \"%s\"", TBL_ATOMIC, CLASSNAME, className);
...
//		}
		...
		ArrayList<Tag> tags = getTagsForComponent(className);
		if(LOG) Log.d(TAG, "Adding " + tags.size() + " tags to " + sd.getClassName());
		sd.addTags(tags);
...
//		ArrayList<ServiceParameter> params = getParametersForAtomic(sd.getClassName());
//		sd.addParameters(params);
	...
	public ArrayList<ServiceDescription> getMatchingForIOs(ServiceDescription component, boolean inputs) 
	{
		...
		for(String s : keys)
		{
			...
			for(int i = 0; i < componentNames.size(); i++)
			{
				...
				{
					components.put(name, getAtomic(name));
				}
			}
		}
	...
	private ArrayList<ServiceDescription> getComponents(ArrayList<Long> ids)
	{
		ArrayList<ServiceDescription> components = new ArrayList<ServiceDescription>();
	...
	private ArrayList<String> getComponentIdsForType(long id, boolean inputs)
	{
		...
		Cursor c = db.query(TBL_SERVICEIO, new String[] { PARENT_SERVICE },
		...
		do
		{
			ioIds.add(c.getString(c.getColumnIndex(PARENT_SERVICE)));
		}
		while(c.moveToNext());
