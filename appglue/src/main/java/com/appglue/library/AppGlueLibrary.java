...
public class AppGlueLibrary 
{
	...
	public static String buildGetAllString(String table, String[] columns)
	{
		...
		{
			out.append(String.format("%s.%s AS %s_%s", table, columns[i], table, columns[i]) + (i < columns.length - 1 ? ", " : " "));
		}
	...
	public static String buildGetAllString(String table, String[][] columns)
	{
		...
		for(int i = 0; i < columns.length; i++)
		{
			out.append(String.format("%s.%s AS %s_%s", table, columns[i][0], table, columns[i][0]) + (i < columns.length - 1 ? ", " : " "));
		}
